...
namespace GraphSharp.Helpers
{
		...
		where T : class, IPoolObject, new()
	{
		...
		{
			Contract.Requires( initialPoolSize > 1 );
		...
		{
			Contract.Ensures( pool.Count == initialPoolSize );
		...
		{
			Contract.Requires( allowPoolGrowth || pool.Count < initialPoolSize );
			Contract.Ensures( pool.Contains( poolObject ) );
		...
		{
			Contract.Requires( sender != null && sender is T );
