...
namespace GraphSharp.Controls
{
        ...
        where TGraph : class, IBidirectionalGraph<TVertex, TEdge>
    {
        ...
        {
            var gl = (GraphLayout<TVertex, TEdge, TGraph>)obj;
            var oldGraph = e.OldValue as TGraph;
            
            //unsubribe events from previous graph
            gl.UnsubscribeEvents(oldGraph);
            
            var newGraph = e.NewValue as TGraph;
            ...
            gl.SubscribeToEvents(newGraph);

            if (gl.HighlightAlgorithm != null)
                gl.HighlightAlgorithm.ResetHighlight();

            //recreate the graph elements
            gl.RecreateGraphElements(false, oldGraph);
