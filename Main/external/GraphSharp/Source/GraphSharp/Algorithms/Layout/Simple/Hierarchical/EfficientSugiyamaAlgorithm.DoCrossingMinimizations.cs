...
namespace GraphSharp.Algorithms.Layout.Simple.Hierarchical
{
        ...
        where TGraph : IVertexAndEdgeListGraph<TVertex, TEdge>
    {
        ...
        {
            Contract.Requires(Math.Abs(step) == 1);
            Contract.Requires(startLayerIndex >= 0);
            Contract.Requires(endLayerIndex < _layers.Count);
            Contract.Requires(startLayerIndex == endLayerIndex || Math.Sign(endLayerIndex - startLayerIndex) == Math.Sign(step));
