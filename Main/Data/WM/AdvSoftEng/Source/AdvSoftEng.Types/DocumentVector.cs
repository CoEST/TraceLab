...
namespace AdvSoftEng.Types
{
    [Serializable]
    [WorkspaceType]
    public class DocumentVector : Dictionary<String, int>, IXmlSerializable
    {

        internal DocumentVector() : base() { }

        public DocumentVector(String name)
        {
            _id = name;
            MaxFreq = new KeyValuePair<string, int>(String.Empty, 0);
        }

        /// <summary>
        /// The maximum frequency word in the vector.
        /// This is NOT updated automatically, you MUST update it yourself if you update the vector frequencies.
        /// Key: word
        /// Value: frequency
        /// </summary>
        public KeyValuePair<string, int> MaxFreq { get; set; }
        ...
        protected DocumentVector(SerializationInfo info, StreamingContext context)
            : base(info, context)
        {
        }
        ...
        public void ReadXml(XmlReader reader)
        {
            ...
            // read MaxFreq
            reader.Read();
            MaxFreq = new KeyValuePair<string, int>(
                reader.ReadElementContentAsString("Key", String.Empty),
                reader.ReadElementContentAsInt("Value", String.Empty)
            );
        ...
        {
            writer.WriteStartElement("DocumentVector");
            writer.WriteAttributeString("Version", Version.ToString());
            writer.WriteElementString("ID", _id);
            // write MaxFreq
            writer.WriteStartElement("MaxFreq");
            writer.WriteElementString("Key", MaxFreq.Key);
            writer.WriteElementString("Value", MaxFreq.Value.ToString());
